...
private:
    ...
    const float GRAVITY = 1900.0f;       // Same as player for consistency
    const float FRICTION = 1500.0f;      // How fast it stops sliding after knockback
    const float KNOCKBACK_FORCE_X = 350.0f;
    const float KNOCKBACK_FORCE_Y = 250.0f; // Small upward pop when hit
